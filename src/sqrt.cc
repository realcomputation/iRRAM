 ...
 *   \f$e[v]=e\f$ for any \ref sizetype representation \f$m\cdot2^e\f$ of
 *   \f$v\f$. The relative precision is \f$\max\{e[\check x]+P,e[z]\}\f$.
 *
 * - Otherwise, \f$x=0\f$ or
 ...
 *
 * \todo
 * For \f$0\in[x_c\pm x_\varepsilon]\f$ use \f$
 *    \approx[\sqrt{x_\varepsilon}\pm\sqrt{x_\varepsilon}]
 * \f$
 * as result to decrease the error and disable representation of non-co-domain
 * values smaller than zero.
 *
 * \todo
 * To determine \f$0\notin[x_c\pm x_\varepsilon]\f$ check \f$
 *    \check x\geq cx_\varepsilon\f$ with \f$c\in\{2,3\}
 * \f$
 * for tighter error bounds in both cases.
 *
 * \todo
 * In case \f$0\notin[x_c\pm x_\varepsilon]\f$, does it make sense to use the
 * bound \f$
 *    x_\varepsilon/(\sqrt{\check x}+\sqrt{\check x-x_\varepsilon})
 * \f$
...
REAL sqrt(const REAL & x)
{
	...
	if (stack.prec_policy == 0)
		local_prec = max(stack.actual_prec, zerror.exponent);
	else
		local_prec = max(xsize.exponent + stack.actual_prec,
		                 zerror.exponent);
	MP_init(zvalue);

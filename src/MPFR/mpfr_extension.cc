...
static DYADIC compute_test(const DYADIC &x, const int &p) { return approx(sin(REAL(x)) ,p); }
...
static DYADIC compute_exp(const DYADIC &x, const int &p) { return approx(exp(REAL(x)),p); }
void mpfr_ext_exp (mpfr_ptr r, mpfr_srcptr u,int p){mpfr_wrapper(compute_exp,r,u,p);}

static DYADIC compute_log(const DYADIC &x, const int &p) { return approx(log(REAL(x)),p); }
void mpfr_ext_log (mpfr_ptr r, mpfr_srcptr u,int p){mpfr_wrapper(compute_log,r,u,p);}

static DYADIC compute_sin(const DYADIC &x, const int &p) { return approx(sin(REAL(x)),p); }
void mpfr_ext_sin (mpfr_ptr r, mpfr_srcptr u,int p){mpfr_wrapper(compute_sin,r,u,p);}

static DYADIC compute_cos(const DYADIC &x, const int &p) { return approx(cos(REAL(x)),p); }
void mpfr_ext_cos (mpfr_ptr r, mpfr_srcptr u,int p){mpfr_wrapper(compute_cos,r,u,p);}

static DYADIC compute_tan(const DYADIC &x, const int &p) { return approx(tan(REAL(x)),p); }

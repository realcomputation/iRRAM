...
REAL one_minus_cos_approx(int prec, int wd, const REAL & x)
{
	...
	INTEGER f = 1;
	int i = 1;
	while (upperbound(xpow[wd - 1]) + upperbound(xp) > prec ||
	       bool(!bound(e, -1))) {
		xp *= xf;
		e = 0;
		for (int j = 0; j < wd; j++) {
...
REAL cos_new2(const REAL & x)
{
	return limit_lip(cos_new_alternative, 0, total_domain, x);
}

//------------------------------------------------------------------------------------

REAL cos_new(const REAL & x)
{
	return limit_lip(cos_new_1, 0, total_domain, x);
}
